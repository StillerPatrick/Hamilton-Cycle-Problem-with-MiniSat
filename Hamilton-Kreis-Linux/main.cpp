// Hamilton-Cycle.cpp : Definiert den Einstiegspunkt f�r die Konsolenanwendung.
//
...
int CallMiniSat(string inputpath , string outputpath){
    ...
    systemcall << "minisat " << inputpath << " " << outputpath ;
...
void initializeEdges(HandleFile*CurrentFile){
    ...
    // 1st Cut the String
        ...
        for (int j = 0 ; j < CurrentFile->getNumOfNodes() ; j++){
            Edges[numofedges].sourceID = atoi(source.c_str());
            Edges[numofedges].sourceTime = j ;
            Edges[numofedges].destinationID = atoi(destination.c_str());
...
void generateCNF() {
    ...
    */
    ...
    alpha << "100" << " 0"<< endl;	/* Node 1 can be visited only at step 0 */
    alpha << "10" << Nodes[n].time <<" 0" << endl;	/* and at step n */
    clause_count += 2;
    ...
    for (int i = 1; i < n; i++) {
        ...
        alpha << "-" << Nodes[i].id << "0" << Nodes[i].time << " 0" << endl;
        clause_count++;
    }
    for (int i = n + 1; i < n * (n + 1); i += n + 1) {
        for (int j = i + 1; j < i + n - 1; j++) {
            for (int k = j + 1; k < i + n; k++) {
                alpha << "-" << Nodes[j].id << "0" << Nodes[j].time << " -" << Nodes[k].id << "0" << Nodes[k].time << " 0" << endl;
                clause_count++;
            }
        }
    }
    ...
    for (int i = n + 1; i < n * (n + 1); i += n + 1) {
        ...
        alpha << "-" << Nodes[i].id << "0" << Nodes[i].time <<" 0" << endl;
        alpha << "-" << Nodes[i + n].id << "0" << Nodes[i + n].time << " 0" <<endl;
        clause_count += 2;
    }
    for (int i = 1; i < n; i++) {
